...
const REGISTRATION_URL = 'https://vidya.co/';
let isFirstRun = true;
...
function createLoginModal() {
  ...
  loginModal.className = 'vidya-login-modal';
  ...
  loginModal.innerHTML = `
    <div class="login-container" style="background: white; padding: 30px; border-radius: 10px; width: 350px; box-shadow: 0 5px 15px rgba(0,0,0,0.3);">
      <div style="text-align: center; margin-bottom: 20px;">
        <img src="${chrome.runtime.getURL('icons/Vidya.png')}" alt="Vidya" style="width: 80px; height: 80px;">
        <h2 style="margin: 10px 0 5px; color: #333;">Login to Vidya</h2>
...
document.addEventListener('mouseup', (e) => {
  ...
  if (selectedText.length > 0 && !tooltip) {
...
chrome.storage.local.get(['userProfile'], (result) => {
  userProfile = result.userProfile || { supportNeeded: true, language: 'English' }; // Add default fields
});
document.addEventListener('mousedown', (e) => {
  if (tooltip && !tooltip.contains(e.target) && (!chatSidebar || !chatSidebar.contains(e.target)) && (!chatContainer || !chatContainer.contains(e.target))) {
    stopLiveAudio(() => {
      tooltip.remove();
      tooltip = null;
...
function createFloatingIcon() {
  chrome.storage.local.get(['isLoggedIn'], (result) => {
    ...
    floatingIcon.src = chrome.runtime.getURL('icons/Vidya1.png');
  ...
  `;
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      x = Math.max(0, Math.min(x, window.innerWidth - floatingIcon.offsetWidth));
...
function addLogoutButton() {
  ...
  const headerActions = chatContainer.querySelector('.vidya-header-actions');
  if (!headerActions) return;

  const logoutBtn = document.createElement('button');
  logoutBtn.className = 'vidya-logout-btn';
...
chrome.storage.local.get(['iconPosition', 'chatState'], (result) => {
  ...
  floatingIcon.className = 'vidya-floating-icon'; // Ensure correct class
...
function setupZIndexObserver() {
  const observer = new MutationObserver((mutations) => {
    mutations.forEach(() => {
      const chatContainer = document.querySelector('.vidya-chat-container');
...
function handleSubOption(mode, type) {
  const input = chatContainer.querySelector('.chat-container-input input');
  const messagesDiv = chatContainer.querySelector('.chat-container-messages');
  ...
  if (mode === 'writing') {
    ...
    const quizInput = chatContainer.querySelector('.quiz-options textarea').value;
    sendChatMessage(null, `Generate a quiz based on: ${quizInput}`);
  } else if (mode === 'translator') {
    const textInput = chatContainer.querySelector('.translator-options textarea').value;
...
function getModeTitle(mode) {
  return {
    ...
    'more-options': 'More Learning Tools',
    'quiz': 'Quiz Generator',
...
function createChatContainer() {
  ...
  chatContainer.className = 'vidya-chat-container';
...
`;
  ...
  chatContainer.innerHTML = `
    <div class="vidya-chat-main">
      <div class="vidya-chat-header">
        <div class="vidya-header-content">
          <div class="vidya-header-avatar">
            <img src="${chrome.runtime.getURL('icons/Vidya1.png')}" alt="Vidya AI" />
          </div>
          <div class="vidya-header-info">
            <h3 id="header-title">Vidya AI Assistant</h3>
            <span class="vidya-status">Online</span>
          </div>
          <div class="vidya-header-act">
          <button class="vidya-close-btn" title="Close">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div class="vidya-header-actions" style="display:none !important;">
          <button class="vidya-home-btn" title="Home" style="display: ${userProfile ? 'inline-flex' : 'none'};">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
              <path d="M10 19v-5h4v5c0 .55.45 1 1 1h3c.55 0 1-.45 1-1v-7h1.7c.46 0 .68-.57.33-.87L12.67 3.6c-.38-.34-.96-.34-1.34 0l-8.36 7.53c-.34.3-.13.87.33.87H5v7c0 .55.45 1 1 1h3c.55 0 1-.45 1-1z"/>
            </svg>
          </button>
          <button class="vidya-settings-btn" title="Settings">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div class="vidya-content-wrapper">
        <div class="vidya-mode-content"></div>
        <div class="vidya-toolbar">
        <button class="vidya-toolbar-btn active" title="Support" data-mode="support">
        ...
        <div class="size-5"><svg width="20" height="20" viewBox="0 0 20 20"  xmlns="http://www.w3.org/2000/svg"><path d="M12.899 1.417H7.1c-.813 0-1.468 0-2 .043-.546.045-1.026.139-1.47.365a3.75 3.75 0 0 0-1.639 1.64c-.226.443-.32.924-.365 1.47-.044.531-.044 1.187-.044 2v10.167c0 .258 0 .5.018.697.02.205.064.475.236.722.221.319.563.532.946.592.297.046.56-.032.752-.105.186-.07.403-.177.634-.29l1.499-.73c.513-.25.708-.343.907-.409q.281-.092.574-.132c.208-.029.424-.03.995-.03h4.755c.813 0 1.468 0 2-.044.546-.044 1.026-.139 1.47-.365a3.75 3.75 0 0 0 1.639-1.639c.226-.444.32-.924.365-1.47.044-.532.044-1.187.044-2V6.935c0-.813 0-1.469-.044-2-.044-.546-.139-1.027-.365-1.47a3.75 3.75 0 0 0-1.639-1.64c-.444-.226-.924-.32-1.47-.365-.532-.043-1.187-.043-2-.043z" ></path><path fill-rule="evenodd" clip-rule="evenodd" d="M5.25 7.113a.75.75 0 0 1 .75-.75h8a.75.75 0 0 1 0 1.5H6a.75.75 0 0 1-.75-.75m0 4.667a.75.75 0 0 1 .75-.75h6a.75.75 0 0 1 0 1.5H6a.75.75 0 0 1-.75-.75" fill="#fff"></path></svg></div>
          ...
          <button class="vidya-toolbar-btn " title="Chat" data-mode="chat" style="display:none !important;">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/>
            </svg>
          </button>
          <button class="vidya-toolbar-btn" title="Writing" data-mode="writing">
          ...
          <button class="vidya-toolbar-btn" title="Lesson Plan" data-mode="lesson-plan">
          <div class="size-5">
 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 60 60"><path fill-rule="evenodd" d="M44.547.97a.5.5 0 0 1 .906 0l1.324 2.837a5 5 0 0 0 2.416 2.416l2.836 1.324a.5.5 0 0 1 0 .906l-2.836 1.324a5 5 0 0 0-2.416 2.416l-1.324 2.836a.5.5 0 0 1-.906 0l-1.324-2.836a5 5 0 0 0-2.416-2.416l-2.836-1.324a.5.5 0 0 1 0-.906l2.836-1.324a5 5 0 0 0 2.416-2.416L44.547.971Zm-32.682 9.26c-.726.049-1.087.138-1.328.238a3.82 3.82 0 0 0-2.07 2.07c-.1.241-.189.601-.238 1.327-.051.746-.052 1.707-.052 3.135s.001 2.39.052 3.135c.05.726.139 1.086.239 1.328a3.82 3.82 0 0 0 2.069 2.069c.241.1.602.189 1.328.239.745.05 1.707.052 3.134.052h35.722c-.853-1.683-1.378-3.633-1.521-5.67a2.177 2.177 0 1 1 4.344-.305c.203 2.9 1.407 5.127 2.794 6.207a2.178 2.178 0 0 1 .045 3.4 3.2 3.2 0 0 1-.575.376 3.2 3.2 0 0 1-1.208.326c-.258.02-.55.02-.786.02H14.926c-1.335 0-2.446 0-3.358-.062-.947-.064-1.837-.203-2.698-.56a8.18 8.18 0 0 1-4.426-4.426c-.356-.86-.495-1.75-.56-2.698-.062-.911-.062-2.022-.062-3.357v-.148c0-1.335 0-2.446.062-3.357.065-.948.204-1.838.56-2.698A8.18 8.18 0 0 1 8.87 6.445c.86-.357 1.75-.496 2.698-.56.912-.062 2.023-.062 3.358-.062h16.457a2.177 2.177 0 1 1 0 4.354H14.999c-1.427 0-2.389.002-3.134.053m41.901 22.592h.048c.237 0 .528 0 .786.021a3.2 3.2 0 0 1 1.209.326q.309.157.574.376a2.177 2.177 0 0 1-.045 3.4c-1.528 1.19-2.823 3.769-2.823 7.055s1.295 5.865 2.823 7.054a2.177 2.177 0 0 1 .045 3.401 3.2 3.2 0 0 1-.574.376c-.465.237-.907.3-1.209.325-.258.022-.55.022-.786.021H14.926c-1.335 0-2.446 0-3.357-.062-.948-.064-1.838-.204-2.699-.56a8.18 8.18 0 0 1-4.425-4.426c-.357-.86-.496-1.75-.56-2.698-.063-.911-.063-2.022-.063-3.357v-.148c0-1.335 0-2.446.062-3.357.065-.948.204-1.838.56-2.698a8.18 8.18 0 0 1 4.426-4.426c.861-.357 1.751-.496 2.699-.56.91-.063 2.022-.063 3.357-.063zM49.16 44c0-2.461.552-4.832 1.56-6.823H15c-1.428 0-2.39.002-3.135.052-.726.05-1.086.14-1.328.24a3.82 3.82 0 0 0-2.069 2.068c-.1.242-.19.602-.239 1.328-.05.746-.052 1.707-.052 3.135s.001 2.39.052 3.135c.05.726.139 1.086.24 1.328a3.82 3.82 0 0 0 2.068 2.069c.242.1.602.189 1.328.239.745.05 1.707.051 3.135.051h35.72c-1.008-1.99-1.56-4.36-1.56-6.822" clip-rule="evenodd"></path></svg>
          ...
          <button class="vidya-toolbar-btn" title="Study Plan" data-mode="study-plan">
          <div class="size-5">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
          ...
          <button class="vidya-toolbar-btn" title="Summarizer" data-mode="summarizer">
          <div class="size-5">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
          ...
          <button class="vidya-toolbar-btn" data-mode="quiz" title="Quiz Generator">
          <div class="size-5">
  ...
  <path d="M16.8594854,20.7417567 C15.3870177,21.5619833 13.7245582,22 12,22 C6.4771525,22 2,17.5228475 2,12 C2,6.4771525 6.4771525,2 12,2 C17.5228475,2 22,6.4771525 22,12 C22,13.7245582 21.5619833,15.3870177 20.7417567,16.8594854 L21.0946904,21.0946904 L16.8594854,20.7417567 Z M16.3916038,18.6958341 L18.9053096,18.9053096 L18.6958341,16.3916038 L18.8621131,16.1149882 C19.6030308,14.8824253 20,13.4715357 20,12 C20,7.581722 16.418278,4 12,4 C7.581722,4 4,7.581722 4,12 C4,16.418278 7.581722,20 12,20 C13.4715357,20 14.8824253,19.6030308 16.1149882,18.8621131 L16.3916038,18.6958341 Z M12.0003283,16.9983464 C11.4478622,16.9983464 11,16.5506311 11,15.9983464 C11,15.4460616 11.4478622,14.9983464 12.0003283,14.9983464 C12.5527943,14.9983464 13.0006565,15.4460616 13.0006565,15.9983464 C13.0006565,16.5506311 12.5527943,16.9983464 12.0003283,16.9983464 Z M13,14 L11,14 L11,13 C11,12.2626932 11.3827392,11.7004784 11.9774877,11.2286498 C12.1564111,11.0867055 12.2101653,11.0510845 12.5339634,10.8458119 C12.8225361,10.6628706 13,10.3473166 13,10 C13,9.44771525 12.5522847,9 12,9 C11.4477153,9 11,9.44771525 11,10 L9,10 C9,8.34314575 10.3431458,7 12,7 C13.6568542,7 15,8.34314575 15,10 C15,11.0395627 14.4660508,11.988994 13.6048137,12.5349773 C13.3517279,12.6954217 13.3206582,12.7160104 13.2204897,12.7954765 C13.0562911,12.9257393 13,13.0084267 13,13 L13,14 Z"/>
        ...
        <button class="vidya-toolbar-btn" data-mode="translator" title="Text Translator">
        <div class="size-5">
        ...
        <g id="Layer_2" data-name="Layer 2">
          ...
          <button class="vidya-toolbar-btn" title="More Options" data-mode="more-options" style="display:none;">
          <div class="size-5">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        ...
        <button class="vidya-toolbar-btn" title="Preferences" data-mode="settings">
        <div class="size-5">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                ...
                <button class="vidya-toolbar-btn" title="Clear Chat" data-mode="clear-chat">
                <div class="size-5">
      ...
      <div class="vidya-settings-panel" style="display: none;"></div>
      <div class="vidya-profile-view" style="display: none;"></div>
      <div class="vidya-voice-container" style="display: none;"></div>
    </div>
  `;
  ...
  const modeContent = chatContainer.querySelector('.vidya-mode-content');
  ...
  const toolbar = chatContainer.querySelector('.vidya-toolbar');
  if (toolbar) {
    const lessonPlanBtn = toolbar.querySelector('[data-mode="lesson-plan"]');
    const studyPlanBtn = toolbar.querySelector('[data-mode="study-plan"]');

    // Set initial visibility based on role
    if (userProfile?.role === 'teacher') {
      if (lessonPlanBtn) lessonPlanBtn.style.display = 'flex ';
      if (studyPlanBtn) studyPlanBtn.style.display = 'none ';
    } else if (userProfile?.role === 'student') {
      if (lessonPlanBtn) lessonPlanBtn.style.display = 'none ';
      if (studyPlanBtn) studyPlanBtn.style.display = 'flex ';
    }
  }
...
function setupChatContainerListeners() {
  const closeBtn = chatContainer.querySelector('.vidya-close-btn');
  const settingsBtn = chatContainer.querySelector('.vidya-settings-btn');
  const homeBtn = chatContainer.querySelector('.vidya-home-btn');
  const toolbarBtns = chatContainer.querySelectorAll('.vidya-toolbar-btn');
  const modeContent = chatContainer.querySelector('.vidya-mode-content');
  const headerTitle = chatContainer.querySelector('#header-title');
...
function switchMode(mode, contentDiv, headerTitle) {
  if (mode === 'support' && userProfile?.supportNeeded === false) {
    contentDiv.innerHTML = '<p class="error">Support mode is disabled. Enable it in Settings.</p>';
  ...
  if (mode === 'clear-chat') {
    ...
    confirmBtn.addEventListener('click', async () => {
      ...
      setTimeout(() => {
        const chatBtn = chatContainer.querySelector('.vidya-toolbar-btn[data-mode="support"]');
        chatBtn.click();
      }, 1500);
    });
  ...
  switch (mode) {
    case 'translator':
      ...
      `;
      document.querySelector('.translate-btn').addEventListener('click', translateText);
      break;
      ...
      case 'quiz':
        contentDiv.innerHTML = `
          ...
          <div  class="quiz-form-inner">
            <div class="form-row">
              ...
              <div class="form-group">
                <label for="quiz-subject">Subject/Topic</label>
                <input type="text" id="quiz-subject" placeholder="e.g., Mathematics, History">
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="question-types-grid">
                <!-- Multiple Choice -->
                <div class="question-type-card active" data-type="mcq">
                    <div class="type-header">
                        ...
                        <div class="quantity-control">
                            <div class="quantity-input-wrapper">
                                <button class="quantity-btn"  data-type="mcq" data-action="decrease">−</button>
                                <span class="quantity-display" id="mcq-qty">5</span>
                                <button class="quantity-btn"   data-type="mcq" data-action="increase">+</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- True/False -->
                <div class="question-type-card active" data-type="tf">
                    <div class="type-header">
                        <div class="type-left">
                            <div class="type-icon">✓</div>
                            <div class="type-title">True / False</div>
                        </div>
                        <div class="quantity-control">
                            <div class="quantity-input-wrapper">
                                <button class="quantity-btn" data-type="tf" data-action="decrease">−</button>
                                <span class="quantity-display" id="tf-qty">3</span>
                                <button class="quantity-btn"  data-type="tf" data-action="increase">+</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Short Answer -->
                <div class="question-type-card active" data-type="short">
                    <div class="type-header">
                        <div class="type-left">
                            <div class="type-icon">💭</div>
                        ...
                        <div class="quantity-control">
                            <div class="quantity-input-wrapper">
                                <button class="quantity-btn"  data-type="short" data-action="decrease">−</button>
                                <span class="quantity-display" id="short-qty">2</span>
                                <button class="quantity-btn"  data-type="short" data-action="increase">+</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Essay -->
                <div class="question-type-card active" data-type="essay">
                    <div class="type-header">
                        <div class="type-left">
                            <div class="type-icon">📄</div>
                            <div class="type-title">Essay</div>
                        </div>
                        <div class="quantity-control">
                            <div class="quantity-input-wrapper">
                                <button class="quantity-btn quantity-btn1" data-type="essay" data-action="decrease">−</button>
                                <span class="quantity-display" id="essay-qty">1</span>
                                <button class="quantity-btn quantity-btn2" data-type="essay" data-action="increase">+</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="form-group">
              <label for="quiz-details">Additional Instructions</label>
              <textarea id="quiz-details" placeholder="Specific topics, difficulty level, time limit, etc."></textarea>
            </div>
        ...
        document.querySelector('.generate-btn').addEventListener('click', generateQuiz);
    ...
    case 'support':    
      contentDiv.innerHTML = `
        ...
        ">
          ${pageContentScraper.supportMessages.length === 0 ? `
            <div class="vidya-intro-message" style="
              text-align: center;
              color: #54656f;
              font-size: 16px;
              font-weight: 500;
            ...
            <h3>Hi,</h3>
              ...
              <p style="margin: 0; font-size: 16px; color: #666;">I'm Vidya, your AI assistant. Ask me anything or select text on the page to get started!</p>
            </div>
          ` : ''}
        </div>
        <div class="chat-container-wrapper">
          <div class="chat-container-input" style="flex-shrink: 0; padding: 10px;">
            <div style="display: flex; align-items: center; gap: 5px; width:100% !important;">
              <div style="position: relative; flex-grow: 1;">
             
                <textarea id="support-input" placeholder="Ask Anything @Support /prompt..." style="
                  width: 100%;
                  ...
                  padding-left: 10px;
                "></textarea>
                <button id="voice-input-btn" style="
                position: absolute;
                left: 85%;
                top: 40%;
      ...
      `;
      const voiceInputBtn = contentDiv.querySelector('#voice-input-btn');
      const voiceStatus = contentDiv.querySelector('#voice-status');
      const supportMessagesDiv = contentDiv.querySelector('.chat-container-messages');
      const supportInput = contentDiv.querySelector('#support-input');
      ...
      supportSendBtn.addEventListener('click', async () => {
        ...
        supportMessagesDiv.style.justifyContent = 'flex-start';
        supportMessagesDiv.style.alignItems = 'stretch';
        const introMessage = supportMessagesDiv.querySelector('.vidya-intro-message');
  ...
  case 'chat':
      contentDiv.innerHTML = `
        <div class="chat-container-messages" style="flex-grow: 1; padding: 10px; overflow-y: auto; max-height: calc(100vh - 150px);"></div>
        <div class="chat-container-input" style="flex-shrink: 0; border-top: 1px solid #e0e0e0;">
      ...
      `;
      const messagesDiv = contentDiv.querySelector('.chat-container-messages');
      ...
      pageContentScraper.loadChatState().then(() => {
        ...
        console.error('Failed to load chat state:', error);
        messagesDiv.innerHTML += '<p class="error">Failed to load previous messages.</p>';
      });
      break;

    case 'writing':
      contentDiv.innerHTML = `
        <div class="writing-form" style="padding: 15px;">
          <h3>Writing Assistant</h3>
          <select class="writing-type">
            <option value="letter">Letter</option>
            <option value="email">Email</option>
            <option value="essay">Essay</option>
          </select>
          <input type="text" class="writing-topic" placeholder="Enter topic or prompt...">
          <input type="text" class="writing-recipient" placeholder="Recipient (optional)">
          <button class="generate-btn">Generate</button>
        </div>
        <div class='writing-preview response-container' style='flex-grow: 1; padding: 10px; background: #f9f9f9;'></div>
      `;
      const writingTopic = contentDiv.querySelector('.writing-topic');
      if (window.getSelection().toString().trim()) writingTopic.value = window.getSelection().toString().trim();
      contentDiv.querySelector('.generate-btn').addEventListener('click', () => generateWriting(contentDiv));
      break;
    ...
    case 'lesson-plan':
      chrome.storage.local.get(['settings'], (result) => {
        ...
        contentDiv.innerHTML = `
          <div class="lesson-form" style="padding: 15px;">
            ...
            <button class="generate-btn">Generate Lesson Plan</button>
          </div>
          <div class="lesson-preview"></div>
        `;
        const generateBtn = contentDiv.querySelector('.generate-btn');
        const previewDiv = contentDiv.querySelector('.lesson-preview');
        ...
        generateBtn.addEventListener('click', async () => {
          ...
          if (!subject || !classLevel || !duration) {
            previewDiv.innerHTML = '<p class="error">Please fill in all required fields.</p>';
            return;
          }

          previewDiv.innerHTML = '<p class="loading">Generating lesson plan...</p>';
          ...
          try {
            ...
            let formattedResponse = data.response
              ...
              .replace(/\n/g, '<br>');
            previewDiv.innerHTML = `<div class="response-content">${formattedResponse}</div>`;
          } catch (error) {
            previewDiv.innerHTML = `<p class="error">Error: ${error.message}</p>`;
          }
        });
      });
      break;
    ...
    case 'study-plan':
      chrome.storage.local.get(['settings'], (result) => {
        const savedSettings = result.settings || { class: '' };
        contentDiv.innerHTML = `
          <div class="study-form" style="padding: 15px;">
            <h3>Study Plan Generator</h3>
            <input type="text" id="subject" placeholder="Enter subject (e.g., Math)">
            <input type="text" id="class" placeholder="Enter your class" value="${savedSettings.class || ''}">
            <input type="text" id="duration" placeholder="Enter duration (e.g., 2 weeks)">
            <input type="text" id="additional-info" placeholder="Enter any additional info(Optional)">
            <button class="generate-btn">Generate Study Plan</button>
          </div>
          <div class="study-preview"></div>
        `;
        const generateBtn = contentDiv.querySelector('.generate-btn');
        const previewDiv = contentDiv.querySelector('.study-preview');
        ...
        generateBtn.addEventListener('click', async () => {
          ...
          previewDiv.innerHTML = '<p class="loading">Generating study plan...</p>';
    ...
    case 'summarizer':
      contentDiv.innerHTML = `
        <div class="summarizer-form" style="padding: 15px;">
          <textarea class="summarizer-input" placeholder="Paste text to summarize..." style="width: 100%; height: 100px; resize: none;"></textarea>
          <div style="margin: 10px 0;">
            <label>Level: <select class="sum-level"><option value="basic">Basic</option><option value="intermediate">Intermediate</option><option value="advanced">Advanced</option></select></label>
          ...
          <button class="generate-btn">Summarize</button>
        </div>
        <div class="summarizer-preview response-container" style="flex-grow: 1; padding: 10px; overflow-y: auto; background: #f9f9f9; max-height: calc(100vh - 250px);"></div>
      `;
      const summarizerInput = contentDiv.querySelector('.summarizer-input');
      if (window.getSelection().toString().trim()) summarizerInput.value = window.getSelection().toString().trim();
      contentDiv.querySelector('.generate-btn').addEventListener('click', () => generateSummary(contentDiv));
      break;
      ...
      contentDiv.innerHTML = `
                        <div class="settings-form">
                            <div class="settings-content">
                                ...
                                <button class="ai-settings-save">Save Settings</button>
                            </div>
                        </div>
                    `;
      function renderSettingsForm() {
        const contentDiv = chatContainer.querySelector('.vidya-mode-content');
        contentDiv.innerHTML = `
                        <div class="settings-form">
                          <div class="settings-content">
                            <div class="ai-settings-field">
                            ...
                            <div class="ai-settings-field" style="display:none !important;">
        ...
        const saveBtn = contentDiv.querySelector('.ai-settings-save');
        ...
        contentDiv.innerHTML = `
                            <div class="vidya-profile-view">
                                <div class="settings-content">
                                    ...
                                    <div class="profile-field" style="display:none !important;">
...
async function generateWriting(contentDiv) {
  const type = contentDiv.querySelector('.writing-type').value;
  const topic = contentDiv.querySelector('.writing-topic').value || window.getSelection().toString().trim();
  const recipient = contentDiv.querySelector('.writing-recipient').value;
  const preview = contentDiv.querySelector('.writing-preview');
  if (!topic) {
    preview.innerHTML = '<p class="error">Please enter a topic or select text.</p>';
    return;
  }
  preview.innerHTML = '<p class="loading">Generating...</p>';
  try {
    const response = await generateAIResponse(topic, { mode: 'writing', type, details: { recipient } });
    preview.innerHTML = `
      <div class="response-content">${response.replace(/\n/g, '<br>')}</div>
      <div class="response-options">
        <button class="copy-btn copy-response" title="Copy"><img src="${chrome.runtime.getURL('icons/copy.png')}" alt="copy" style="width:14px;height:14px;"></button>
    ...
    addResponseOptionsListeners(preview, true, () => generateWriting(contentDiv));
  } catch (error) {
    preview.innerHTML = `<p class="error">Error generating response: ${error.message}</p>`;
  }
}
...
function changeQuantity(type, delta) {
  ...
  const card = document.querySelector(`.question-type-card[data-type="${type}"]`);
...
async function generateQuiz() {
  ...
  const form = document.querySelector('.quiz-form-inner');
  if (!container) return;
  ...
  const subject = document.querySelector('#quiz-subject').value || 'General Knowledge';
  const details = document.querySelector('#quiz-details').value || '';
...
function renderQuizQuestion() {
      ...
      questionHTML = `
        <div class="quiz-question">
          <div class="question-header">
            <span class="question-count">Question ${questionNumber}/${totalQuestions}</span>
    ...
    case 'short_answer':
      questionHTML = `
        <div class="quiz-question">
          ...
          <div class="question-text">${question.question}</div>
          <textarea class="short-answer" placeholder="Type your answer here...">${quizState.userAnswers[quizState.currentIndex] || ''}</textarea>
        </div>
      `;
      break;
    ...
    case 'essay':
      questionHTML = `
        <div class="quiz-question">
          ...
          <div class="question-text">${question.question}</div>
          <textarea class="essay-answer" placeholder="Type your essay here...">${quizState.userAnswers[quizState.currentIndex] || ''}</textarea>
        </div>
      `;
      break;
  }
  ...
  const navigationHTML = `
    <div class="quiz-navigation">
      <button class="nav-btn prev-btn" ${quizState.currentIndex === 0 ? 'disabled' : ''}>
    ...
    <button class="exit-quiz-btn">Exit Quiz</button>
  `;
  ...
  document.querySelector('.exit-quiz-btn').addEventListener('click', confirmExitQuiz);
}
...
function saveAnswer() {
  ...
  if (question.type === 'multiple_choice' || question.type === 'true_false') {
    ...
    answer = document.querySelector('.short-answer, .essay-answer').value;
  }
  ...
  try {
    ...
    const prompt = `
      ...
      Response format:
      {
        ...
        "badge": "Badge name based on performance",
...
function showQuizResults(result) {
  ...
  const badgeIcon = badgeIcons[result.badge] || '🎓';
    ...
    <div class="quiz-results">
      <div class="result-header">
        <div class="score-badge">
          ...
          <div class="badge-name">${result.badge}</div>
        </div>
      ...
      <div class="result-actions">
        <button class="action-btn retry-quiz">Retry Quiz</button>
        <button class="action-btn new-quiz">New Quiz</button>
        <button class="action-btn exit-quiz">Exit</button>
      </div>
    </div>
  `;
  ...
  document.querySelector('.exit-quiz').addEventListener('click', resetQuiz);
}
...
async function generateRoleplay() {
  ...
  container.innerHTML = '<p class="loading">Generating roleplay...</p>';
  ...
  try {
    ...
    container.innerHTML = `
      ...
      <div class="roleplay-actions">
        <button class="roleplay-action-btn" data-action="add-roles">Add More Roles</button>
        <button class="roleplay-action-btn" data-action="extend">Extend Scenario</button>
        <button class="roleplay-action-btn" data-action="simplify">Simplify Language</button>
     
      </div>
    `;

    // Add action listeners
    document.querySelectorAll('.roleplay-action-btn').forEach(btn => {
...
async function translateText() {
  ...
  if (!sourceText.trim()) {
    resultContainer.innerHTML = '<p class="error">Please enter text to translate</p>';
    return;
  }
  ...
  try {
    ...
    resultContainer.innerHTML = `
      <div class="translation-output">
        <div class="translated-text">${data.response}</div>
        <div class="translation-actions">
          <button class="copy-translation">Copy Translation</button>
          <button class="speak-translation">Speak Translation</button>
    ...
    // Add action listeners
    resultContainer.querySelector('.copy-translation').addEventListener('click', () => {
      copyText(data.response);
      showCopyFeedback(resultContainer.querySelector('.copy-translation'), true);
    });

    resultContainer.querySelector('.speak-translation').addEventListener('click', () => {
      speakText(data.response);
    });

    resultContainer.querySelector('.new-translation').addEventListener('click', () => {
...
function handleQuizAction(e) {
  ...
  const quizContent = document.querySelector('.quiz-content').innerText;
  const subject = document.querySelector('#quiz-subject').value;
  ...
  preview.innerHTML = '<p class="loading">Processing...</p>';
  ...
  fetch('https://ai.learneng.app/LearnEng/generate', {
    ...
    .then(data => {
      let formattedResponse = data.response
        ...
        .replace(/Answer:\s*(.*)/g, '<div class="quiz-answer">Answer: $1</div>')
        .replace(/Explanation:\s*(.*)/g, '<div class="quiz-explanation">Explanation: $1</div>')
        .replace(/\n/g, '<br>');

      preview.innerHTML = `
      <div class="quiz-content">${formattedResponse}</div>
      <div class="quiz-actions">
        <button class="quiz-action-btn" data-action="generate-more">Generate More Questions</button>
        <button class="quiz-action-btn" data-action="simplify">Simplify Questions</button>
        <button class="quiz-action-btn" data-action="add-details">Add More Details</button>
...
async function generateLessonPlan(contentDiv) {
  const subject = contentDiv.querySelector('.lesson-subject').value;
  const grade = contentDiv.querySelector('.lesson-grade').value;
  const duration = contentDiv.querySelector('.lesson-duration').value;
  const preview = contentDiv.querySelector('.lesson-preview');
  if (!subject) {
    preview.innerHTML = '<p class="error">Please enter a subject.</p>';
    return;
  }
  preview.innerHTML = '<p class="loading">Generating...</p>';
  try {
    const response = await generateAIResponse('Generate a lesson plan', { mode: 'lesson-plan', details: { subject, grade, duration } });
    preview.innerHTML = `
      <div class="response-content">${response.replace(/\n/g, '<br>')}</div>
      <div class="response-options">
        <button class="copy-btn copy-response" title="Copy"><img src="${chrome.runtime.getURL('icons/copy.png')}" alt="copy" style="width:18px;height:18px;"></button>
    ...
    addResponseOptionsListeners(preview, true, () => generateLessonPlan(contentDiv));
  } catch (error) {
    preview.innerHTML = `<p class="error">Error generating response: ${error.message}</p>`;
  }
}

async function generateStudyPlan(contentDiv) {
  const classLevel = contentDiv.querySelector('.study-class').value;
  const subject = contentDiv.querySelector('.study-subject').value;
  const topic = contentDiv.querySelector('.study-topic').value || window.getSelection().toString().trim();
  const timeline = contentDiv.querySelector('.study-timeline').value;
  const preview = contentDiv.querySelector('.study-preview');
  if (!topic) {
    preview.innerHTML = '<p class="error">Please enter a topic or select text.</p>';
    return;
  }
  preview.innerHTML = '<p class="loading">Generating...</p>';
  try {
    const response = await generateAIResponse(topic, { mode: 'study-plan', details: { class: classLevel, subject, topic, timeline } });
    const rows = response.split('\n').map(line => {
      const cells = line.split('|').slice(1, -1).map(cell => `<td>${cell.trim()}</td>`);
      return cells.length ? `<tr>${cells.join('')}</tr>` : '';
    }).filter(row => row);
    preview.innerHTML = `
      <div class="response-content">${rows.length ? `<table>${rows.join('')}</table>` : response.replace(/\n/g, '<br>')}</div>
      <div class="response-options">
        <button class="copy-btn copy-response" title="Copy"><img src="${chrome.runtime.getURL('icons/copy.png')}" alt="copy" style="width:14px;height:14px;"></button>
        <button class="speaker-btn" title="Speak"><img src="${chrome.runtime.getURL('icons/speak.png')}" alt="speak"  style="width:14px;height:14px;></button>
        <button class="edit-btn" title="Edit"><img src="${chrome.runtime.getURL('icons/edit.png')}" alt="edit"  style="width:14px;height:14px;></button>
    ...
    addResponseOptionsListeners(preview, true, () => generateStudyPlan(contentDiv));
  } catch (error) {
    preview.innerHTML = `<p class="error">Error generating response: ${error.message}</p>`;
  }
}
...
async function generateSummary(contentDiv) {
  const text = contentDiv.querySelector('.summarizer-input').value || window.getSelection().toString().trim();
  const level = contentDiv.querySelector('.sum-level').value;
  const paragraphs = contentDiv.querySelector('.sum-paragraphs').value;
  const preview = contentDiv.querySelector('.summarizer-preview');
  if (!text) {
    preview.innerHTML = '<p class="error">Please enter text or select some to summarize.</p>';
    return;
  }
  preview.innerHTML = '<p class="loading">Generating...</p>';
  try {
    const response = await generateAIResponse(text, { mode: 'summarizer', details: { level, paragraphs } });
    preview.innerHTML = `
      <div class="response-content">${response.replace(/\n/g, '<br>')}</div>
      <div class="response-options">
        <button class="copy-btn copy-response" title="Copy"><img src="${chrome.runtime.getURL('icons/copy.png')}" alt="copy"  style="width:14px;height:14px;"></button>
...
function formatWriting(type, topic, recipient) {
  switch (type) {
...
function startVoiceChatMode() {
  ...
  const voiceContainer = chatContainer.querySelector('.voice-input-container');
  const messagesDiv = chatContainer.querySelector('.chat-container-messages');
  const inputDiv = chatContainer.querySelector('.chat-container-input');
  const settingsPanel = chatContainer.querySelector('.settings-panel1');
  const profileView = chatContainer.querySelector('.profile-view');
  ...
  voiceContainer.innerHTML = `
    <img src="${chrome.runtime.getURL('icons/Vidya.png')}" alt="Listening" class="state-icon listening" style="width: 100px; height: 100px; border-radius: 50%;">
    <div class="voice-status" style="margin-top: 10px; padding: 8px 15px; background-color: rgba(0,0,0,0.7); color: white; border-radius: 4px;">Listening...</div>
    ...
    <div class="voice-controls" style="margin-top: 20px; display: flex; gap: 10px; justify-content: center;">
      <button class="cancel-voice-btn" style="background: #f44336; color: #fff; padding: 8px 15px; border: none; border-radius: 4px; cursor: pointer;">Cancel</button>
  ...
  `;
  chatContainer.classList.add('voice-input-active');

  // References to dynamic UI elements
  const transcribedDiv = voiceContainer.querySelector('.transcribed-text');
  const statusDiv = voiceContainer.querySelector('.voice-status');
  const cancelBtn = voiceContainer.querySelector('.cancel-voice-btn');
  ...
  const handleVoiceInteraction = (firstTime = true) => {
    ...
    if (!firstTime) {
      ...
      const speakingBars = voiceContainer.querySelector('.speaking-bars');
      if (speakingBars) {
        const stateIconHTML = `<img src="${chrome.runtime.getURL('icons/Vidya.png')}" alt="Listening" class="state-icon listening" style="width: 100px; height: 100px; border-radius: 50%;">`;
      ...
      async (transcript) => {
        ...
        try {
          ...
          if (currentStateIcon && currentStateIcon.parentNode) {
            ...
            speakingBarsElement.className = 'speaking-bars';
  ...
  cancelBtn.addEventListener('click', () => {
    ...
    stopLiveAudio(() => {
      returnToChatMode();
      chatContainer.classList.remove('voice-input-active');
    });
  });
...
function returnToChatMode() {
  ...
  chatContainer.classList.remove('voice-input-active'); // Reset background
}
...
function startVoiceSettings() {
  ...
  <img src="${chrome.runtime.getURL('icons/Vidya.png')}" alt="Listening" class="state-icon">
  <div class="voice-question"></div>
...
`;
  ...
  const questionDiv = voiceContainer.querySelector('.voice-question');
  const transcribedDiv = voiceContainer.querySelector('.transcribed-text');
  ...
  function updateVoiceUI(state, transcript = '') {
    ...
    if (state === 'listening') {
      stateIcon.src = chrome.runtime.getURL('icons/Vidya.png');
      stateIcon.alt = 'Listening';
      responseDiv.textContent = "Listening...";
    } else if (state === 'processing') {
      stateIcon.src = chrome.runtime.getURL('icons/Vidya.png');
      stateIcon.alt = 'Processing';
  ...
  function animateSpeakingBars() {
    if (!tooltip) return; // Exit if tooltip is null
    const bars = tooltip.querySelectorAll('.speaking-bars .bar');
    ...
    const animate = () => {
      ...
      if (isSpeaking && tooltip) requestAnimationFrame(animate);
    };
  ...
  function listenForAnswer() {
    startVoiceRecognition(
      (transcript) => {
        ...
        updateVoiceUI('processing', transcript);
          ...
          (transcript) => {
            ...
            if (transcript.toLowerCase().includes('yes')) {
              ...
              chrome.storage.local.set({ userProfile }, () => {
                ...
                showProfileView();
                voiceContainer.style.display = 'none';
                const homeBtn = chatContainer.querySelector('.home-btn');
  ...
  function setupControlButtons() {
    ...
    function handleSkip() {
      stopLiveAudio(() => {
        updateVoiceUI('processing', 'Skipped');
...
function startVoiceRecognition(inputElement, statusElement) {
  ...
  recognition.onerror = (event) => {
    ...
    isVoiceInputActive = false;
    document.querySelector('#voice-input-btn').innerHTML =
...
function saveSettings() {
  ...
  userProfile = { name, age, class: classGrade, language, role, image };
  chrome.storage.local.set({ userProfile }, () => {
    console.log('Profile saved:', userProfile);
    showProfileView();
    const homeBtn = chatContainer.querySelector('.home-btn');
    ...
    // Refresh the toolbar buttons when role changes
    const modeContent = chatContainer.querySelector('.vidya-mode-content');
    const headerTitle = chatContainer.querySelector('#header-title');
    const currentMode = chatContainer.querySelector('.vidya-toolbar-btn.active')?.getAttribute('data-mode') || 'support';
...
function showProfileView() {
  ...
  profileView.innerHTML = `
    <div class="whatsapp-profile-view" style="display: flex; flex-direction: column; padding: 15px; height: 100%; box-sizing: border-box; position: relative;">
      <button class="edit-profile" style="position: absolute; top: 10px; left: 80%; background: none; border: none; padding: 5px; cursor: pointer; display: flex; align-items: center; gap: 5px; transition: background 0.2s;">
  ...
  const editBtn = profileView.querySelector('.edit-profile');
  editBtn.addEventListener('click', () => {
    profileView.style.display = 'none';
  ...
  if (settingsPanel.style.display === 'flex') {
    ...
    settingsPanel.innerHTML = `
      <div class="whatsapp-profile-settings" style="display: flex; flex-direction: column; padding: 15px; height: 100%; overflow-y: auto; box-sizing: border-box;">
        ...
        <div class="profile-field" style="margin-bottom: 10px; flex-shrink: 0;">
        ...
        <div class="profile-field" style="margin-bottom: 15px; flex-shrink: 0;">
...
function addResponseOptionsListeners(container, includeRetry, retryCallback) {
  ...
  const contentDiv = container.querySelector('.message-content, .response-content');
  ...
  editBtn.addEventListener('click', () => {
    ...
    textarea.addEventListener('blur', () => {
      ...
      div.className = container.querySelector('textarea') ? 'message-content' : 'response-content';
...
async function sendSupportMessage(question, messagesDiv) {
  ...
  aiMessageDiv.innerHTML = '<div class="message-content"><p class="loading">Generating...</p></div>';
...
And say hi or greetings if it is related to the user context or question.
Your task is to:
...
Response Format (JSON):
{
  ...
  "recommendedQuestions": ["Follow-up question 1", "Follow-up question 2", "Follow-up question 3"]
    ...
    aiMessageDiv.innerHTML = `<div class="message-content"><p class="error">Error generating response: ${error.message}</p></div>`;
  }
  messagesDiv.scrollTop = messagesDiv.scrollHeight;
...
function generateDefaultQuestions(userContext) {
  ...
  if (userContext.role === 'student') {
    questions = [
      ...
      "How can I use this for my studies?",
...
function addMessageToChat(role, content, messagesDiv, recommendedQuestions = []) {
  ...
  if (questionsToShow.length > 0) {
    ...
    questionsDiv.className = 'recommended-questions';
...
async function sendChatMessage(role, content, messagesDiv) {
  ...
  messagesDiv.scrollTop = messagesDiv.scrollHeight;
  chatContainer.querySelector('.chat-container-input input').value = '';
}

async function callOpenAIApi(selectedText, option) {
  const interactionContainer = tooltip.querySelector('#interaction-container');
  interactionContainer.innerHTML = '<div class="loading">Processing...</div>';
  ...
  try {
    ...
    interactionContainer.innerHTML = `<div class="error">Error: ${error.message}</div>`;
  }
}
...
function togglePin(option, selectedText, pinnedOptions) {
  ...
  const pinnedContainer = tooltip.querySelector('.pinned-options');
  ...
  const pinBtn = tooltip.querySelector(`.pin-btn[data-option="${option}"]`);
  if (pinBtn) {
...
function showTooltip(e, selectedText) {
  ...
  chrome.storage.local.get(['pinnedOptions'], (result) => {
    ...
    tooltip.innerHTML = `
      <div class="button-row">
        <button class="ask-ai-btn" title="Ask AI">
          <img src="${chrome.runtime.getURL('icons/Vidya.png')}" alt="AI">
        </button>
        <button class="copy-btn copy-response" title="Copy Text">
    ...
    requestAnimationFrame(() => {
      const tooltipWidth = Math.min(tooltip.offsetWidth, viewportWidth * 0.5);
      const tooltipHeight = tooltip.offsetHeight;
      let topPosition = rect.bottom + scrollY + 5;
      let leftPosition = rect.left + scrollX;

      if (leftPosition + tooltipWidth > scrollX + viewportWidth - 10) {
        leftPosition = scrollX + viewportWidth - tooltipWidth - 10;
      }
...
function setupMenuOptions(selectedText, pinnedOptions) {
  const menuBtn = tooltip.querySelector('.menu-btn');
  const menuOptions = tooltip.querySelector('.menu-options');
  const options = [
  ...
  menuBtn.onclick = (e) => {
    ...
    if (menuOptions.innerHTML === '' && !isVisible) {
      ...
      // Adjust tooltip position when menu-options are shown
      requestAnimationFrame(() => {
        const tooltipWidth = tooltip.offsetWidth;
        const menuOptionsWidth = menuOptions.offsetWidth;
        const totalWidth = tooltipWidth + menuOptionsWidth + 8; // Include gap
        const currentLeft = parseFloat(tooltip.style.left);

        if (currentLeft + totalWidth > scrollX + viewportWidth) {
          tooltip.style.left = `${scrollX + viewportWidth - totalWidth - 5}px`;
        }
      });
...
function updateInteractionContainer(selectedText) {
  if (!tooltip) return;
  const container = tooltip.querySelector('#interaction-container') || document.createElement('div');
  container.id = 'interaction-container';
  container.innerHTML = `
    <div id="mode-content"></div>
  `; // Removed mode-toggle since text is default
  if (!tooltip.querySelector('#interaction-container')) tooltip.appendChild(container);
  updateModeContent(selectedText);
}
function updateModeContent(selectedText) {
  if (!tooltip) return;
  const modeContent = tooltip.querySelector('#mode-content');
  ...
  if (isVoiceMode) {
    modeContent.innerHTML = `
      <div class="live-status-container">
        <div class="ai-icon-wrapper">
          <img src="${chrome.runtime.getURL('icons/Vidya.png')}" alt="AI" class="ai-icon listening">
        </div>
        <div class="live-controls">
          <button class="stop-live-btn" style="display:none !important;">Stop</button>
          <button class="cancel-voice-btn" style="
          background: #f44336;
          color: #fff;
          border: none;
          border-radius: 6px;
        ...
        <button class="ask-again-btn" style="
          background: #2196F3;
          color: #fff;
          border: none;
          border-radius: 6px;
    ...
    const cancelBtn = modeContent.querySelector('.cancel-voice-btn');
    if (cancelBtn) {
      cancelBtn.addEventListener('click', () => {
        stopLiveAudio(() => {
          isVoiceMode = false;
    ...
    modeContent.innerHTML = `
      <div class="prompt-actions" style="margin-top:2%;">
        <input type="text" id="prompt-input" placeholder="Ask about '${selectedText.substring(0, 20)}...'">
...
function setupEventListeners(selectedText) {
  const askAiBtn = tooltip.querySelector('.ask-ai-btn');
  const copyBtn = tooltip.querySelector('.copy-btn');
  const speakBtn = tooltip.querySelector('.speak-btn');
  const pinnedContainer = tooltip.querySelector('.pinned-options');
...
function setupToggleListeners(selectedText) {
  if (!tooltip) return;
  const toggleBtns = tooltip.querySelectorAll('.toggle-btn');
...
function setupTextModeListeners(selectedText) {
  if (!tooltip) return;
  const submitBtn = tooltip.querySelector('.submit-prompt');
...
async function startLiveAudio(selectedText) {
  ...
  if (!SpeechRecognition) {
    const modeContent = tooltip.querySelector('#mode-content');
    if (modeContent) modeContent.innerHTML = '<div class="error">Speech Recognition not supported</div>';
    return;
  }
  ...
  const listen = () => {
    stopLiveAudio(() => {
      ...
      recognition.onstart = () => {
        isRecording = true;
        if (tooltip) updateVoiceUI(selectedText, 'listening');
      };
      ...
      recognition.onerror = (event) => {
        ...
        stopLiveAudio(() => {
          if (tooltip) {
            const modeContent = tooltip.querySelector('#mode-content');
            if (modeContent) modeContent.innerHTML = `<div class="error">Error: ${event.error}</div>`;
          }
        });
      };
...
function stopLiveAudio(callback) {
  ...
  const bars = document.querySelectorAll('.speaking-bars .bar');
  if (bars.length > 0) {
...
function updateVoiceUI(selectedText, state) {
  ...
  if (state === 'listening') {
    ...
    iconHTML = `
      <img src="${chrome.runtime.getURL('icons/Vidya.png')}" alt="AI" class="ai-icon loading">
    `;
  } else if (state === 'speaking') {
    iconHTML = `
      <div class="speaking-bars">
  ...
  modeContent.innerHTML = `
    <div class="live-status-container">
      ...
      <div class="status-text">${state === 'listening' ? 'Listening...' : state === 'loading' ? 'Processing...' : 'Speaking...'}</div>
      <div class="live-controls">
      <button class="cancel-voice-btn" style="
      background: #f44336;
      color: #fff;
...
function setupLiveControls(selectedText) {
  if (!tooltip) return;
  const cancelBtn = tooltip.querySelector('.cancel-voice-btn');
  const askAgainBtn = tooltip.querySelector('.ask-again-btn');
...
async function processSpeech(selectedText, transcript) {
  ...
  try {
    ...
    if (tooltip) {
      const modeContent = tooltip.querySelector('#mode-content');
      if (modeContent) modeContent.innerHTML = `<div class="error">Error: ${error.message}</div>`;
    }
  }
}
function speakResponse(text, selectedText) {
  return new Promise((resolve, reject) => {
    if (!('speechSynthesis' in window) || !tooltip) {
      reject('Speech Synthesis not supported or tooltip missing');
      return;
    }
    ...
    stopLiveAudio(() => {
      ...
      function speakNextChunk() {
        if (chunkIndex >= textChunks.length || !tooltip) {
          isSpeaking = false;
          currentUtterance = null;
          if (tooltip) updateVoiceUI(selectedText, 'listening');
          resolve();
        ...
        currentUtterance.onstart = () => {
          isSpeaking = true;
          if (tooltip) updateVoiceUI(selectedText, 'speaking');
        };
...
function showCopyFeedback(button, success) {
  ...
  button.className = originalClasses + (success ? ' copy-success' : ' copy-error');
...
async function askAI(selectedText, prompt) {
  if (!tooltip) return;
  const responseContainer = tooltip.querySelector('#response-container');
  if (responseContainer) responseContainer.innerHTML = '<div class="loading">Thinking...</div>';
  ...
  try {
    ...
    if (tooltip && responseContainer) responseContainer.innerHTML = `<div class="error">Error: ${error.message}</div>`;
  }
}
function showResponse(data, selectedText) {
  ...
  responseContainer.innerHTML = `
      ...
      <div class="response-actions">
          <button class="copy-btn copy-response" title="Copy">
  ...
  const copyBtn = responseContainer.querySelector('.copy-btn.copy-response');
  ...
  const responseText = responseContainer.querySelector('.response-content').textContent;
  ...
  if (retryBtn) {
    retryBtn.addEventListener('click', () => {
      ...
      if (promptInput) askAI(selectedText, promptInput.value);
        ...
        const lastOption = tooltip.querySelector('.menu-option .option-btn:focus')?.getAttribute('data-option') || 'translate';
...
function setupResponseListeners(selectedText) {
  ...
  const responseContainer = tooltip.querySelector('#response-container');
  if (!responseContainer) return;
  const copyBtn = tooltip.querySelector('.copy-response');
  const retryBtn = tooltip.querySelector('.retry-response');
  const speakBtn = tooltip.querySelector('.speak-response');
  let continueBtn = tooltip.querySelector('.continue-chat');
  const responseText = tooltip.querySelector('.response-content').textContent;
...
function showChatSidebar(selectedText, previousResponse = null) {
  ...
  chatSidebar.innerHTML = `
    ...
    <div class="chat-content">${previousResponse ? `<div class="ai-message">${previousResponse.response}</div>` : ''}</div>
