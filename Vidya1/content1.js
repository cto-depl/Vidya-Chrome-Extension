...
function startVoiceToVoiceMode() {
  ...
  recognition.onstart = () => {
    isRecording = true;
    console.log('Voice-to-voice: Listening...');
  };

  recognition.onresult = async (event) => {
    const transcript = event.results[0][0].transcript;
    console.log('Voice-to-voice Input:', transcript);

    // Send to existing support processing logic
    const response = await processSupportChatPrompt(transcript);

    // Speak response
    speakResponse1(response);
    isRecording = false;
  };
